// Source: ~/claudecode/openclaudecode/src/tools/TaskCreateTool/prompt.ts
use crate::utils::agent_swarms::is_agent_swarms_enabled;

pub const DESCRIPTION: &str = "Create a new task in the task list";

pub fn get_prompt() -> String {
    let teammate_context = if is_agent_swarms_enabled() {
        " and potentially assigned to teammates"
    } else {
        ""
    };

    let teammate_tips = if is_agent_swarms_enabled() {
        r#"- Include enough detail in the description for another agent to understand and complete the task
...
"#,
        teammate_context, teammate_tips,
    )
}
