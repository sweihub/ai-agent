...
pub struct AgentOptions {
    ...
    /// Thinking configuration - enables extended thinking
    /// Defaults to Adaptive if not specified (matches TypeScript shouldEnableThinkingByDefault)
    #[serde(default)]
    pub thinking: Option<ThinkingConfig>,
    /// Callback for agent events (tool start/complete/error, thinking, done)
    /// This is not serializable, so only available at runtime
    #[serde(skip)]
