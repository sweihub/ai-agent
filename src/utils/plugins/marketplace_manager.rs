...
use super::schemas::{is_marketplace_auto_update, MarketplaceSource};
...
pub async fn load_known_marketplaces_config() -> Result<HashMap<String, super::types::KnownMarketplace>, Box<dyn std::error::Error + Send + Sync>> {
    let config_file = get_known_marketplaces_file();

    match fs::read_to_string(&config_file).await {
        Ok(content) => {
            let data: HashMap<String, super::types::KnownMarketplace> = serde_json::from_str(&content)?;
            Ok(data)
        }
...
pub async fn save_known_marketplaces_config(
    ...
    let dir = config_file.parent().ok_or_else(|| "Invalid config path".to_string())?;
...
pub async fn get_marketplace(name: &str) -> Result<PluginMarketplace, Box<dyn std::error::Error + Send + Sync>> {
    {
...
pub async fn add_marketplace_source(source: &MarketplaceSource) -> Result<AddMarketplaceResult, Box<dyn std::error::Error + Send + Sync>> {
    // Stub: simplified
