 ...
 */
use ai_agent::{
    PermissionContext, PermissionHandler, PermissionMode, PermissionRule,
    PermissionBehavior, PermissionMetadata,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Default mode: {:?}", result.message().unwrap_or(&"no message".to_string()));

    // Bypass mode - allows everything
    let ctx = PermissionContext::new().with_mode(PermissionMode::Bypass);
    let result = ctx.check_tool("Bash", None);
    println!("Bypass mode: {}", if result.is_allowed() { "allowed" } else { "denied" });

    // DontAsk mode - denies everything
    let ctx = PermissionContext::new().with_mode(PermissionMode::DontAsk);
    let result = ctx.check_tool("Bash", None);
    println!("DontAsk mode: {}", if result.is_denied() { "denied" } else { "allowed" });
    ...
    println!("AcceptEdits mode - Write: {}, Read: {}",
        if write_result.is_allowed() { "allowed" } else { "denied" },
        if read_result.is_allowed() { "allowed" } else { "denied" }
    );
    ...
    println!("Allow rule for Read: {}", if result.is_allowed() { "allowed" } else { "denied" });
    ...
    let result = ctx.check_tool("Bash", None);
    println!("Deny rule for Bash: {}", if result.is_denied() { "denied" } else { "allowed" });
    ...
    println!("Ask rule for Grep: {}", if result.is_ask() { "asks" } else { "auto" });
    ...
    println!("Without callback: {}", result.message().unwrap_or(&"allowed".to_string()));
    ...
    let result = handler.check(metadata.clone());
    println!("With rules: {}", if result.is_allowed() { "allowed" } else { "denied" });
    ...
    let ctx = PermissionContext::new()
        .with_allow_rule(PermissionRule::with_content("Bash", PermissionBehavior::Allow, "ls"));

    let input1 = serde_json::json!({"command": "ls -la"});
    let result = ctx.check_tool("Bash", Some(&input1));
    println!("Bash with 'ls': {}", if result.is_allowed() { "allowed" } else { "denied" });

    let input2 = serde_json::json!({"command": "rm -rf /"});
    let result = ctx.check_tool("Bash", Some(&input2));
    println!("Bash with 'rm': {}", if result.is_allowed() { "allowed" } else { "denied" });
